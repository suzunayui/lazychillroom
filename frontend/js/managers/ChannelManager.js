...
class ChannelManager {
    ...
    // チャンネルまたはDMに切り替え
    async switchToChannel(channelId, type = 'guild') {
        try {
            if (type === 'dm') {
                // DM チャンネルに切り替え
                const dmManager = window.chatUI?.dmManager;
                if (dmManager) {
                    const dmChannel = dmManager.findDMChannelById(channelId);
                    if (dmChannel) {
                        await dmManager.switchToDMChannel(dmChannel);
                        return true;
                    }
                }
                return false;
                ...
                if (window.chatUI) {
                    ...
                    window.chatUI.currentChannel = channel;
                    await window.chatUI.loadAndRenderMessages(channelId);
            ...
            console.error('チャンネル切り替えエラー:', error);
            return false;
        }
    }
}
